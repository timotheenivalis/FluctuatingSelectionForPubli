...
%' 
 ...
 <<EvolSmooth,dev="tikz",fig.height=7,fig.width=10>>=
setPar()
plot(x=0,xlim=c(2006,2016),ylim=c(-5,5),type="n", xlab="Year",ylab="Mean breeding values for BMI")
trashidontwantyou<-lapply(bvplotlist, function(x){lines(x[,1],x[,2], col=rgb(0.1,0.1,0.1,alpha = 0.1))})
abline(h=0)
@
...
setPar()
plot(x=SD_Beta_Levels, y=unlist(lapply(rhopv, function(x){mean(x<0.075)})), xlab="") #1.5 df
@
\end{document}
