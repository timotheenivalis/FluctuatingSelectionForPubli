...
setPar()
plot(x=SD_Beta_Levels, y=unlist(lapply(fitnesspv, function(x){mean(x<0.075)})), ylim=c(0,1), type="b", lty=2, xlab = "$\\abs{\\beta_{z}^\\prime}/\\sigma_{\\zeta}$", ylab = "Power") #1.5 df
...
yest <- 0.5
points(x=attr(summary(ZmmARRfitness)$varcor[[1]], "stddev")["BMIst"]/abs(summary(ZmmARRfitness)$coefficients["BMIst",1]), y=yest, pch=16)
...
setPar()
plot(x=SD_Beta_Levels, y=unlist(lapply(rhopv, function(x){mean(x<0.075)})), xlab="$\\abs{\\beta_{z}^\\prime}/\\sigma_{\\zeta}$", ylab = "Power", type = "b", lty=2) #1.5 df
...
yest <- 0.5
points(x=attr(summary(ZmmRrho)$varcor[[1]], "stddev")["BMIst"]/abs(summary(ZmmRrho)$coefficients["BMIst",1]), y=yest, pch=16)
