...
SigRat <- c(sqrt(as.numeric(smmARnoCorfitness$varcor$Year.1))/smmARnoCorfitness$coefficients[2,1],
            sqrt(as.numeric(smmRnoCorrho$varcor$Year.1))/smmRnoCorrho$coefficients[2,1],
sqrt(as.numeric(smmRnoCorphi$varcor$Year.1))/smmRnoCorphi$coefficients[2,1])
