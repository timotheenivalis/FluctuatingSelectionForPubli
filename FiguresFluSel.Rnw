...
<<concetpualplot,dev="tikz",fig.height=18,fig.width=14>>=
mat <- matrix(c(1, 2, 3, 4, 5, 6),ncol = 2)
layout(mat, widths = rep.int(1, ncol(mat)), heights = c(1,1,1), respect = FALSE )

szgr <- 2
szax <- 1.3

par(las=1, cex=szgr, cex.lab=szax , cex.axis=szax, lwd=2 ,pch=16, las=1,
    oma=c(0,0,0,0))
...
legend(x = "topleft", legend = c("Selection differential", "Genetic differential", "Evolution"),
              bty = "n",
       col = c(grey(seq(0.3^2.2, 0.9^2.2, length = 2)^(1/2.2)),"black"),
       lty = c(0, 0, 1), lwd = c(0, 0, 3),
       pch = c(22, 22, NA),
       pt.bg = c(grey(seq(0.3^2.2, 0.9^2.2, length = 2)^(1/2.2)),NA),
       pt.cex = 3, cex=1.5)
#cex=1, fill = , xjust=0.5, yjust=0.5, lty=c(0,0,1), lwd = c(0, 0, 1))

### Graph decoupling 
par(mar=c(3, 1, 2, 4) + 0.1)
dec_selcov <- c(1.2,0.82,0.6,0.94,1.3,0.65)
dec_evol <- h2* dec_selcov - 0.4

dec_tp <- as.matrix(rbind(dec_selcov, dec_evol))

cumulative_dec <- cumsum(dec_evol)

xb4 <- barplot(height = dec_tp, ylim = c(-1.5,2), beside = TRUE,
               main="\\textbf{(D)}", yaxt="n", border = NA)
